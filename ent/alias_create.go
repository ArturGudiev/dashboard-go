...
func (_c *AliasCreate) SetID(v int) *AliasCreate {
	_c.mutation.SetID(v)
	return _c
}
...
func (_c *AliasCreate) check() error {
	if _, ok := _c.mutation.GetType(); !ok {
		return &ValidationError{Name: "type", err: errors.New(`ent: missing required field "Alias.type"`)}
	}
	if v, ok := _c.mutation.GetType(); ok {
		if err := alias.TypeValidator(v); err != nil {
	...
	if v, ok := _c.mutation.ID(); ok {
		if err := alias.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`ent: validator failed for field "Alias.id": %w`, err)}
		}
	}
	return nil
}
