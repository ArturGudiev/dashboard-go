...
// topLayoutShift returns a Y delta so the finder block is drawn from the top of the screen.
func (f *finder) topLayoutShift(height int) int {
	if f.opt == nil || !f.opt.alignTop {
		return 0
	}
	maxHeight := height
	maxHeight--
	...
	if visible == 0 {
		...
		blockTop = maxHeight - 1 - (visible - 1)
	}
	return -blockTop
}
...
func (f *finder) _draw() {
	...
	for _, r := range f.opt.promptString {
		...
		f.term.SetContent(promptLinePad, maxHeight-1+dy, r, nil, style)
		promptLinePad++
	}
	var r rune
	var w int
	for _, r = range f.state.input {
		...
		f.term.SetContent(promptLinePad+w, maxHeight-1+dy, r, nil, style)
		w += runewidth.RuneWidth(r)
	}
	f.term.ShowCursor(promptLinePad+f.state.cursorX, maxHeight-1+dy)
	...
	if len(f.opt.header) > 0 {
		...
		for _, r := range runewidth.Truncate(f.opt.header, maxWidth-2, "..") {
			style := tcell.StyleDefault.
				...
				Background(tcell.ColorDefault)
			f.term.SetContent(2+w, maxHeight-1+dy, r, nil, style)
			w += runewidth.RuneWidth(r)
		}
		maxHeight--
	}
	...
	for i, r := range fmt.Sprintf("%d/%d", len(f.state.matched), len(f.state.items)) {
		...
		f.term.SetContent(2+i, maxHeight-1+dy, r, nil, style)
	}
	maxHeight--
	...
	for i, m := range matched {
		...
		if i == f.state.cursorY {
			...
			f.term.SetContent(0, maxHeight-1-i+dy, '>', nil, style)
			f.term.SetContent(1, maxHeight-1-i+dy, ' ', nil, style)
		}
		...
		for j, r := range []rune(f.state.items[m.Idx]) {
			...
			// Shorten item cells.
			if w+rw+2 > maxWidth {
				f.term.SetContent(w, maxHeight-1-i+dy, '.', nil, style)
				f.term.SetContent(w+1, maxHeight-1-i+dy, '.', nil, style)
				break
			} else {
				f.term.SetContent(w, maxHeight-1-i+dy, r, nil, style)
				w += rw
			}
		}
	}
